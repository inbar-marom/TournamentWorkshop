...
public class SignalREventPublisherTests
{
    ...
    [Fact] public async Task UpdateCurrentStateAsync_UpdatesStateAndNotifiesClients() { var state = new DashboardStateDto { TournamentState = new TournamentStateDto { Status = TournamentStatus.InProgress }, Message = "Tournament active" }; await _publisher.UpdateCurrentStateAsync(state); _mockClientProxy.Verify( x => x.SendCoreAsync( "CurrentState", It.Is<object[]>(args => args.Length == 1 && args[0] == state), default), Times.Once); }
